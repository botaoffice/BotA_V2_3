...
void init_CopyCat_BOT(string type,string ti_type){
   ...
   ArrayResize(CopyCat_BOT,0);
      ...
      for(int i=0;i<ArraySize(Garen_BOT);i++){
         ...
         if(Detail == 3){
            ...
            //---------------------------------------------------------------------------------------------
                     //      |Name                        |  Period  |
            create_CopyCat_BOT(Garen_BOT[i].get_name() +"_CC"+string(j)+string(k) + "_T" + string(l) + string(m)  + string(n) + string(o) + string(p)+ string(q)+string(r)  , Garen_BOT[i].get_P(),
                      
                      //   ExcuseTime| UseDark  | UseFed | UseFastClose |                     UseBEClose
                                Garen_BOT[i].get_Excuse_TIME()  , false    ,   fed   ,   9999999, Garen_BOT[i].get_UseBEClose(),  useghostcarry,
...
void create_CopyCat_BOT(string Name,ENUM_TIMEFRAMES P,
               ...
               int UseActiveTarget,int UseDeactiveTarget){
